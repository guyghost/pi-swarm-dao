...
import { statusLabel } from "./governance/lifecycle.js";
import { transitionProposal, buildContext } from "./shell/lifecycle-manager.js";
import {
...
import {
  classifyRiskZone,
  formatZoneClassification,
} from "./governance/zones.js";
import {
  validateAmendmentPayload,
  previewAmendment,
  executeAmendment,
  rollbackAmendment,
} from "./governance/amendments.js";
import {
...
export default function daoExtension(pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, _ctx) => {
    ...
    const riskCounts = agents.reduce(
      (acc, a) => {
    ...
    if (hostCtx.framework)
      daoContext += `\n- **Framework:** ${hostCtx.framework}`;
    if (hostCtx.packageManager)
      daoContext += `\n- **Package Manager:** ${hostCtx.packageManager}`;
    if (hostCtx.isSelfRepo)
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      agentId: Type.Optional(
        Type.String({
          description:
            "Agent ID (for agent-update, agent-remove, council-update)",
        }),
      ),
      agentChanges: Type.Optional(
        Type.String({
          description:
            'JSON object of agent fields to change (for agent-update). E.g. \'{"weight": 4, "role": "new role"}\'',
        }),
      ),
      // agent-add fields
      newAgentId: Type.Optional(
        Type.String({ description: "New agent ID (for agent-add)" }),
      ),
      newAgentName: Type.Optional(
        Type.String({ description: "New agent name (for agent-add)" }),
      ),
      newAgentRole: Type.Optional(
        Type.String({ description: "New agent role (for agent-add)" }),
      ),
      newAgentDescription: Type.Optional(
        Type.String({ description: "New agent description (for agent-add)" }),
      ),
      newAgentWeight: Type.Optional(
        Type.Number({
          description: "New agent weight 1-10 (for agent-add)",
          minimum: 1,
          maximum: 10,
        }),
      ),
      // config-update fields
      configChanges: Type.Optional(
        Type.String({
          description:
            "JSON object of config fields to change (for config-update). E.g. '{\"quorumPercent\": 65}'",
        }),
      ),
      // quorum-update fields
      quorumChanges: Type.Optional(
        Type.String({
          description:
            'JSON object of per-type quorum changes (for quorum-update). E.g. \'{"governance-change": {"quorumPercent": 75}}\'',
        }),
      ),
      // gate-update fields
      addGates: Type.Optional(
        Type.Array(Type.String(), {
          description: "Gate IDs to add (for gate-update)",
        }),
      ),
      removeGates: Type.Optional(
        Type.Array(Type.String(), {
          description: "Gate IDs to remove (for gate-update)",
        }),
      ),
      // council-update fields
      councils: Type.Optional(
        Type.String({
          description:
            'JSON array of council memberships (for council-update). E.g. \'[{"council": "governance-council", "role": "lead"}]\'',
        }),
      ),
      // Origin
      originSource: Type.Optional(
        StringEnum(["human", "agent"], {
          description: "Who initiated this amendment (default: human)",
        }),
      ),
      originAgentId: Type.Optional(
        Type.String({
          description: "Agent ID that initiated (if originSource is agent)",
        }),
      ),
    }),
    promptSnippet:
      "dao_propose_amendment — Propose a self-amendment to the DAO (agents, config, quorum, gates, councils)",
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      const state = getState();
      if (!state.initialized) {
      ...
      try {
        switch (params.amendmentType) {
          case "agent-update": {
            if (!params.agentId)
              return toolResult("Error: agentId is required for agent-update");
            if (!params.agentChanges)
              return toolResult(
                "Error: agentChanges JSON is required for agent-update",
              );
          ...
          case "agent-add": {
            if (
              ...
              !params.newAgentName ||
              !params.newAgentRole ||
              !params.newAgentDescription ||
              !params.newAgentWeight
            ) {
              return toolResult(
                "Error: newAgentId, newAgentName, newAgentRole, newAgentDescription, newAgentWeight are all required for agent-add",
              );
            }
            payload = {
              type: "agent-add",
          ...
          case "agent-remove": {
            if (!params.agentId)
              return toolResult("Error: agentId is required for agent-remove");
            payload = { type: "agent-remove", agentId: params.agentId };
            break;
          }
          case "config-update": {
            if (!params.configChanges)
              return toolResult(
                "Error: configChanges JSON is required for config-update",
              );
          ...
          case "quorum-update": {
            if (!params.quorumChanges)
              return toolResult(
                "Error: quorumChanges JSON is required for quorum-update",
              );
          ...
          case "gate-update": {
            if (!params.addGates?.length && !params.removeGates?.length) {
              return toolResult(
                "Error: at least one of addGates or removeGates is required for gate-update",
              );
            }
            payload = {
              type: "gate-update",
              addGates: params.addGates,
              removeGates: params.removeGates,
            };
            break;
          }
          case "council-update": {
            if (!params.agentId)
              return toolResult(
                "Error: agentId is required for council-update",
              );
            if (!params.councils)
              return toolResult(
                "Error: councils JSON is required for council-update",
              );
          ...
          default:
            return toolResult(
              `Error: unknown amendment type: ${params.amendmentType}`,
            );
        }
      } catch (err: any) {
      ...
      proposal.amendmentState =
        origin.source === "agent" ? "pending-vote" : "pending-vote";
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      proposalId: Type.Number({
        description: "ID of the amendment proposal to preview",
      }),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      if (!proposal.amendmentPayload) {
        return toolResult(
          `Proposal #${params.proposalId} is not an amendment proposal.`,
        );
      }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      proposalId: Type.Number({
        description: "ID of the amendment proposal to approve and execute",
      }),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      const proposal = getProposal(params.proposalId);
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      weight: Type.Optional(
        Type.Number({
          description: "New vote weight (1-10)",
          minimum: 1,
          maximum: 10,
        }),
      ),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      quorumPercent: Type.Optional(
        Type.Number({
          description: "New quorum percentage",
          minimum: 1,
          maximum: 100,
        }),
      ),
      approvalThreshold: Type.Optional(
        Type.Number({
          description: "New approval threshold percentage",
          minimum: 1,
          maximum: 100,
        }),
      ),
      riskThreshold: Type.Optional(
        Type.Number({
          description: "New risk threshold (1-10)",
          minimum: 1,
          maximum: 10,
        }),
      ),
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      const changes: Record<string, any> = {};
      if (params.quorumPercent !== undefined)
        changes.quorumPercent = params.quorumPercent;
      if (params.approvalThreshold !== undefined)
        changes.approvalThreshold = params.approvalThreshold;
      if (params.riskThreshold !== undefined)
        changes.riskThreshold = params.riskThreshold;
      ...
      if (Object.keys(changes).length === 0) {
        return toolResult(
          "No changes specified. Provide at least one config field to update.",
        );
      }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      type: StringEnum(PROPOSAL_TYPES, {
        description: "Proposal type category",
      }),
      ...
      context: Type.Optional(
        Type.String({
          description:
            "Additional context (market data, constraints, prior decisions)",
        }),
      ),
      problemStatement: Type.Optional(
        Type.String({
          description:
            "Structured problem statement: what problem does this solve and for whom?",
        }),
      ),
      acceptanceCriteria: Type.Optional(
        Type.Array(Type.String(), {
          description:
            "Measurable, testable conditions that must be met for the proposal to be considered successful. E.g. ['Test coverage >= 80%', 'Response time < 500ms']",
        }),
      ),
      successMetrics: Type.Optional(
        Type.Array(Type.String(), {
          description:
            "Quantitative metrics to track after execution. E.g. ['deliberation_latency < 60s', 'proposal_throughput >= 5/day']",
        }),
      ),
      rollbackConditions: Type.Optional(
        Type.Array(Type.String(), {
          description:
            "Measurable triggers for rollback. E.g. ['Test suite failure rate > 10%', 'Performance regression > 20%']",
        }),
      ),
    }),
    promptSnippet:
      "dao_propose — Create a new DAO proposal for swarm deliberation",
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      const state = getState();
      if (!state.initialized) {
        return toolResult("DAO not initialized. Run `dao_setup` first.");
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      proposalId: Type.Number({
        description: "ID of the proposal to deliberate on",
      }),
    }),
    promptSnippet:
      "dao_deliberate — Run full swarm deliberation with weighted voting",
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (onUpdate) {
        onUpdate({
          content: [
            {
              type: "text" as const,
              text: `🗳️ Starting deliberation on proposal #${proposal.id}: ${proposal.title}...`,
            },
          ],
          details: {},
        });
      ...
      ctx?.ui?.setWorkingMessage?.(
        "DAO: Deliberating with 10 agents (\u23F3 ~3-6 min)...",
      );
      ...
      const votes = agentOutputs.map((output) => {
        ...
        if (output.content) {
          const parsed = parseVoteFromOutput(
            output.agentId,
            output.agentName,
            weight,
            output.content,
          ...
          if (
            parsed.position !== "abstain" ||
            parsed.reasoning !== "No vote section found in agent output"
          ) {
            return parsed;
          }
        }
      ...
      if (proposal.content) {
        compositeScore = applyMalus(
          compositeScore,
          proposal.content.permissionsImpact,
          proposal.content.dataImpact,
        );
      }
      ...
      if (onUpdate) {
        onUpdate({
          content: [
            {
              type: "text" as const,
              text: `🗂️ Generating delivery plan for proposal #${proposal.id}: ${proposal.title}...`,
            },
          ],
          details: {},
        });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      proposalId: Type.Number({
        description: "ID of the proposal to view artefacts for",
      }),
    ...
    promptSnippet:
      "dao_artefacts — View auto-generated artefacts (Decision Brief, ADR, Risk Report, PRD, Plan, Tests, Release)",
    async execute(_toolCallId, params, _signal, onUpdate, _ctx) {
      const state = getState();
      const proposal = getProposal(params.proposalId);
      if (!proposal) {
        ...
        if (onUpdate) {
          onUpdate({
            content: [
              {
                type: "text" as const,
                text: `📚 Generating artefacts for proposal #${proposal.id}...`,
              },
            ],
            details: {},
          });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      proposalId: Type.Optional(
        Type.Number({
          description: "ID of the proposal to audit (omit for all)",
        }),
      ),
    }),
    promptSnippet:
      "dao_audit — View audit trail for a proposal or all proposals",
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      const state = getState();
      ...
      if (params.proposalId !== undefined) {
        ...
        if (entries.length === 0) {
          return toolResult(
            `No audit entries found for proposal #${params.proposalId}.`,
          );
        }
    ...
    async handler(_args: string, ctx: ExtensionCommandContext) {
      if (!ctx.hasUI) {
        pi.sendMessage({
          ...
          content:
            "Interactive proposal creation requires UI mode. Use the `dao_propose` tool instead.",
          display: true,
        });
        return;
      }
      ...
      const selectedType = await ctx.ui.select("Proposal Type", PROPOSAL_TYPES);
  ...
  registerDaoCommandAliases(["dao:config"], {
    ...
    async handler(_args: string, ctx: ExtensionCommandContext) {
      ...
      const configMap: Record<
        string,
        { key: keyof typeof state.config; parse: (v: string) => any }
      > = {
        Quorum: {
          key: "quorumPercent",
          parse: (v) => Math.max(0, Math.min(100, parseInt(v, 10) || 60)),
        },
        Approval: {
          key: "approvalThreshold",
          parse: (v) => Math.max(0, Math.min(100, parseInt(v, 10) || 51)),
        },
        Default: { key: "defaultModel", parse: (v) => v },
        Max: {
          key: "maxConcurrent",
          parse: (v) => Math.max(1, Math.min(8, parseInt(v, 10) || 4)),
        },
        Risk: {
          key: "riskThreshold",
          parse: (v) => Math.max(1, Math.min(10, parseInt(v, 10) || 7)),
        },
      };
    ...
    async handler(args: string, ctx: ExtensionCommandContext) {
      ...
      for (const r of results) {
        if (r.error) {
          ...
          const emoji =
            r.proposal.status === "approved" ? "✅ Approved" : "❌ Rejected";
          lines.push(`## ${emoji} — #${r.proposal.id} ${r.proposal.title}\\n`);
          lines.push(`| Metric | Value |`);
          lines.push(`|--------|-------|`);
          lines.push(`| Score | ${r.compositeScore?.weighted ?? "?"}/100 |`);
          lines.push(
            `| Votes | ${r.tally.weightedFor}/${r.tally.totalVotingWeight} weighted for |`,
          );
          lines.push(
            `| Quorum | ${r.tally.quorumMet ? "✅ Met" : "❌ Not met"} |`,
          );
  ...
  registerDaoCommandAliases(["dao:roundtable"], {
    ...
    async handler(_args: string, ctx: ExtensionCommandContext) {
      ...
      if (ctx.hasUI) {
        ctx.ui.notify(
          `🗣️ Round table starting — ${state.agents.length} agents...`,
          "info",
        );
      }
      ...
      try {
        ...
        const hostCtx = detectHostContext();
        const formatted = formatRoundTable(
          suggestions,
          proposalIds,
          proposalTitles,
          hostCtx.repoSlug,
  ...
  pi.registerCommand("dao:ship", {
    description:
      "Run the full DAO pipeline on a proposal: deliberate → check → execute. Pass a proposal ID or leave empty for first open proposal.",
    async handler(args: string, ctx: ExtensionCommandContext) {
      const state = getState();
      if (!state.initialized) {
        pi.sendMessage({
      ...
      if (trimmed && !isNaN(numericId)) {
        ...
        const openProposals = listProposals().filter(
          (p) => p.status === "open",
        );
        if (openProposals.length === 0) {
          pi.sendMessage({
      ...
      try {
        ...
        if (!skipDeliberate) {
          ...
          if (proposal.content) {
            compositeScoreResult = applyMalus(
          ...
          const tallyTransitionResult = await transitionProposal(
            ...
            buildContext(proposal.id, {
              status: "deliberating",
              quorumMet: deliberationStatus === "approved",
          ...
          ghAddDeliberation(
            ...
            agentOutputs,
            {
              weightedFor: tallyResult.weightedFor,
              weightedAgainst: tallyResult.weightedAgainst,
              totalVotingWeight: tallyResult.totalVotingWeight,
              votingAgents: tallyResult.votingAgents,
              totalAgents: tallyResult.totalAgents,
        ...
        if (!skipCheck) {
          ...
          recordAudit(
            proposal.id,
            "control",
            controlResultValue.allGatesPassed ? "gates_passed" : "gates_failed",
            "system",
            `Ship pipeline: ${controlResultValue.blockerCount} blockers, ${controlResultValue.warningCount} warnings`,
          );

          if (!controlResultValue.allGatesPassed) {
            const failedGates = controlResultValue.gates.filter(
              (g: any) => !g.passed && g.severity === "blocker",
            );
            pi.sendMessage({
              customType: "dao-ship-result",
              content:
  ...
  registerDaoCommandAliases(["dao:hello"], {
    ...
    async handler(_args: string, ctx: ExtensionCommandContext) {
      ...
      agentIntro +=
        "These are the 10 agents that will analyze your proposals:\n\n";
      ...
      agentIntro +=
        "\n**Total weight:** " +
        agents.reduce((s, a) => s + a.weight, 0) +
        "\n\n";
      agentIntro += "> Higher weight = more influence on the final decision.\n";
      agentIntro +=
        "> Agents vote **for**, **against**, or **abstain** — and explain their reasoning.\n\n";
      ...
      if (ctx.hasUI) {
        try {
          projectName =
            (await ctx.ui.input("What's your project name?", "my-project")) ??
            "my-project";

          const goalChoice = await ctx.ui.select("What's your primary goal?", [
            "Ship a feature",
      ...
      const typeLabel =
        PROPOSAL_TYPE_LABELS[
          proposalType as keyof typeof PROPOSAL_TYPE_LABELS
        ] || proposalType;

      const starterDescription =
        `Auto-generated starter proposal for **${projectName}**.\n\n` +
  ...
  registerDaoCommandAliases(["dao:quickstart"], {
    ...
    async handler(_args: string, ctx: ExtensionCommandContext) {
      ...
      try {
        ...
        ctx.ui.setStatus(
          "dao-quickstart",
          "Step 2/4: Deliberating (10 agents, ~3 min)...",
        );
        ctx.ui.notify(
          "Step 2/4: Deliberating with 10 agents (~3 min)...",
          "info",
        );
        ...
        pi.sendMessage({
          ...
          content:
            ...
            votes
              .map(
                (v) =>
                  `| ${v.agentName} | ${v.position === "for" ? "✅" : v.position === "against" ? "❌" : "⏸️"} ${v.position} | ${v.weight} |`,
              )
              .join("\n") +
            "\n\n" +
        ...
        ctx.ui.setStatus(
          "dao-quickstart",
          "Step 3/4: Running control gates...",
        );
        const controlResult = runGates(proposal);
        const checklist = generateChecklist(proposal);
        controlResult.checklist = checklist;
        ...
        const plan = parseDeliveryPlan(
          proposal.id,
          proposal.agentOutputs.find((o) => o.agentId === "delivery")
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      topic: Type.Optional(
        Type.String({
          description:
            "Optional topic to focus agent suggestions (e.g., 'UX improvements', 'security')",
        }),
      ),
    }),
    promptSnippet:
      "dao_roundtable — Agents suggest ideas that become proposals automatically",
    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      const state = getState();
      if (!state.initialized) {
        return toolResult("DAO not initialized. Run `dao_setup` first.");
      ...
      ctx?.ui?.setWorkingMessage?.(
        "DAO: Round table — collecting ideas from " +
          agents.length +
          " agents...",
      );
      ...
      if (onUpdate) {
        onUpdate({
          content: [
            {
              type: "text" as const,
              text: `🗣️ Round table starting — ${agents.length} agents suggesting ideas...`,
            },
          ],
          details: {},
        });
          ...
          if (onUpdate) {
            onUpdate({
              content: [
                {
                  type: "text" as const,
                  text: `🗣️ ${completed}/${total} — ${agentName} responded`,
                },
              ],
              details: {},
            });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      expectedFiles: Type.Optional(
        Type.Array(Type.String(), {
          description: "Files expected to have been changed",
        }),
      ),
    }),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      commits: Type.Array(Type.String(), {
        description: "List of commit SHAs that implemented the proposal",
      }),
      ...
      branch: Type.Optional(
        Type.String({ description: "Feature branch name" }),
      ),
    }),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      proposalId: Type.Number({
        description: "ID of the executed proposal to rate",
      }),
      score: Type.Union(
        [
          Type.Literal(1),
        ...
        {
          description:
            "Rating: 1=failure, 2=below expectations, 3=met expectations, 4=exceeded, 5=far exceeded",
        },
      ),
      comment: Type.String({
        description: "Why this rating? What was the actual outcome?",
      }),
      metricName: Type.Optional(
        Type.String({
          description:
            "Optional: metric name to track (e.g., 'deliberation_latency')",
        }),
      ),
      metricBefore: Type.Optional(
        Type.String({ description: "Optional: metric value before" }),
      ),
      metricAfter: Type.Optional(
        Type.String({ description: "Optional: metric value after" }),
      ),
    }),
      ...
      if (onUpdate) {
        onUpdate({
          content: [
            {
              type: "text" as const,
              text: `🧪 Running dry-run on proposal #${params.proposalId}: ${proposal.title}...`,
            },
          ],
          details: {},
        });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      proposalId: Type.Number({
        description: "ID of the proposal to rollback",
      }),
    }),
    promptSnippet:
      "dao_rollback — Revert proposal execution to pre-execution snapshot",
    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      const proposal = getProposal(params.proposalId);
      if (!proposal) {
        return toolResult(`Proposal #${params.proposalId} not found.`);
      ...
      if (onUpdate) {
        onUpdate({
          content: [
            {
              type: "text" as const,
              text: `⏪ Rolling back proposal #${params.proposalId}: ${proposal.title}...`,
            },
          ],
          details: {},
        });
